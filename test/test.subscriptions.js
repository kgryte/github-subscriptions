...
var get = require( './../lib/subscriptions.js' );
...
tape( 'function returns an error to a provided callback if an error is encountered when fetching a list of watched repositories', function test( t ) {
	var opts;
	var get;

	get = proxyquire( './../lib/subscriptions.js', {
		'./factory.js': factory
	});
...
tape( 'function supports providing only a `token` option (get the authenticated user\'s watched repos)', function test( t ) {
	var expected;
	var opts;
	var get;
...
tape( 'function supports providing both `token` and `username` options to get another user\'s watched repos (increased rate limits)', function test( t ) {
	var expected;
	var opts;
	var get;
